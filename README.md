...
- **배포**: GitHub Pages. 라이브 주소 **https://dgunest.github.io/** (저장소 `main` 브랜치의
  루트를 그대로 서빙). dongguk.edu 커스텀 도메인(`nestlab.dongguk.edu`) 연결은 신청 진행 중.
- **언어**: 한/영 KOR·ENG 토글 지원 (상단 우측 버튼). 아래 "한/영 이중언어" 참고.
...
├─ docs/                 # 설계·계획 문서 (superpowers/specs, superpowers/plans, i18n 스펙)
...
이름(`.name`)과 직책(`.pos`)을 수정합니다. **이름은 한글 그대로, 직책은 영문**으로 통일합니다
(예: `Post-doctoral Researcher`, `Integrated M.S./Ph.D. Student`, `M.S. Student`).

```html
<div class="member"><div class="photo"></div><div class="name">허수경</div><div class="pos">Post-doctoral Researcher</div></div>
```

사진이 있다면 `assets/img/people/`에 이미지 파일을 넣고, `<div class="photo"></div>`를
`<div class="photo"><img src="assets/img/people/파일명.jpg" alt="이름"></div>`로 교체합니다.

### PI 정보 수정
`people.html`의 `#pi` 블록에서 이름(`<h3>`)·소속(`.role`)·소개 문구(`<p>`)·이메일/Google Scholar
링크(그 아래 `<a href>`)를 수정합니다. 국문·영문이 함께 들어 있으니 양쪽 다 확인하세요.

### Contact 정보·지도 수정
`contact.html`에서 이메일·주소를 수정합니다. **지도 위치**는 `<iframe class="map" src="...q=위도,경도...">`
의 `q=` 좌표와, 바로 아래 "지도에서 열기" 링크(`<a class="map-link" ... ll=위도,경도 ...>`)의 `ll=`
좌표를 같은 값으로 바꾸면 됩니다.

### 탭 마크(favicon) 교체
`assets/img/favicon.svg`(또는 `favicon.png`)를 새 파일로 덮어씁니다. 모든 페이지 `<head>`가 이미
이 파일을 참조하므로 파일만 바꾸면 전 페이지 탭 아이콘이 바뀝니다. 정사각형 이미지를 권장합니다.
...
위쪽에 둡니다. 캡션에 한글이 들어가면 아래 "한/영 이중언어"처럼 `lang-ko`/`lang-en` span으로
감싸 주세요 (사진은 `assets/img/gallery/`에 보관).

### 이미지 교체 규칙
지금은 모든 이미지가 자리표시자(`<div class="ph">이미지</div>` 또는 `<div class="photo">...</div>`)
입니다. 실제 이미지가 준비되면:

1. 용도별 폴더에 파일을 넣습니다: 구성원 `assets/img/people/`, 연구 `assets/img/research/`,
   갤러리 `assets/img/gallery/`.
2. 자리표시자 `<div class="ph">…</div>`를 `<img src="assets/img/폴더/파일명" alt="설명">`로 바꿉니다.
